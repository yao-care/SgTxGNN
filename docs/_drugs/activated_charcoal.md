...
Activated charcoal is a non-selective gastrointestinal adsorbent classically used as an emergency decontaminant in acute drug overdose and poisoning.
The TxGNN model predicts it may be effective for **Jeune Syndrome Situs Inversus** (a rare ciliopathy causing skeletal dysplasia with visceral organ laterality defects), with a prediction score of **89.94%**.
However, this prediction is currently supported by **0 clinical trials** and **0 publications**, and the mechanistic rationale is considered biologically implausible — the prediction likely reflects a knowledge graph network artifact rather than a genuine therapeutic signal.
