...
Chlortetracycline (Aureomycin) is the world's first tetracycline antibiotic, historically developed for the treatment of broad-spectrum bacterial infections.
The TxGNN model predicts it may have potential utility in **Postinfectious Vasculitis** — a condition characterised by persistent vascular inflammation following infection — with a prediction score of **99.82%**.
However, there are currently **no clinical trials and no published literature** directly supporting this specific indication, meaning this prediction rests entirely on model inference.
